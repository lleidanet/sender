...
class SenderTest extends TestCase
{
    ...
    public function test_base64()
    {
        ...
        $this->assertTrue(
            $this->instance->public_isBase64Encoded($b64)
        );
        $this->assertFalse(
            $this->instance->public_isBase64Encoded(file_get_contents(dirname(__DIR__) . "/examples/image.png"))
        );
    }
    ...
    public function test_toBool()
    {
        ...
        foreach ($test_true as $test) {
            $this->assertTrue($this->instance->public_toBool($test), "True? ". $test);
        }
        foreach ($test_false as $test) {
            $this->assertFalse($this->instance->public_toBool($test), "False? ". $test);
        }
    }
}
